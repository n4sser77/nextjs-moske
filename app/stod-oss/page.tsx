export default function StodOss() {
    return (
        <div className="md:w-10/12 lg:w-8/12 mx-auto sm:w-full ">
            <h1 className="text-2xl bg-white text-slate-700 font-bold text-center">Stöd oss</h1>
            <hr className="m-2 w-48 mx-auto border border-slate-200 border-2"/>
            
            <div className="mx-auto  w-10/12 lg:mx-auto sm:w-full lg:w-6/1 p-5 m-8">
                Kommer snart. <br /><br />

                Här kommer du smidigt kunna stöda oss genom att donera. <br/>
                 Det ska enklare att ge gåvor i form av sadaqa, zakat, zakat-al-fitr eller annat.
                Tills dess donation sker som vanligt via swish.
            </div>
        </div>
    )
};
